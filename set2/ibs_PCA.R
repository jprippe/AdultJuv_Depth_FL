...
setwd(paste0('~/Google Drive/Work/~Research/~Projects_Active/AdultJuv_Depth_FL/work/set2/', spp)) # change this to where your scp'd files are
...
ibsMat <- as.matrix(read.table(paste0(prefix,"0.ibsMat")))
...
write.table(bamlist,file="bams_noclones",row.names=F,col.names=F,quote=F) # use this bam list to rerun ANGSD for ADMIXTURE
...
ggsave(paste0(prefix, 'PCA_k', npops, '_admix_noLines.png'), height = 6, width = 8)
