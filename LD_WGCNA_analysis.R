...
load(paste0('set1/', spp, '/LDsquare_datt_traits_0618.RData'))
...
message(length(prune.all)) #1641 (mcav), 3035 (ssid)
if(ld.stat == 'DEM'){
...
####### Change colors for SSID #######
mergedColors[mergedColors=='green'] <- 'yellow'
colnames(mergedMEs)[colnames(mergedMEs)=='MEgreen'] <- 'MEyellow'
######################################
...
if(spp == 'mcav'){
  Traits=traits[,c("is1", "is2", "is3", "is4", "isA", "isN", "isO", "isD")]
  traitlabels <- c("Near", "Off", "Deep1", "Deep2", "Age", "Near", "Off", "Deep")
} else if(spp == 'ssid'){
